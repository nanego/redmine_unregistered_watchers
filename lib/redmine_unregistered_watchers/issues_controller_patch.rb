  ...
  module IssuesControllerPatch
    def set_unregistered_watchers
      ...
      if params[:issue] && params[:issue][:unregistered_watchers].present?
        emails = Array.wrap(params[:issue][:unregistered_watchers]).flat_map { |value| value.to_s.split(',') }
        @unregistered_watchers = emails.map(&:strip).reject(&:blank?).uniq.map do |email|
          UnregisteredWatcher.new(email: email)
        end
...
class IssuesController < ApplicationController
  ...
  prepend RedmineUnregisteredWatchers::IssuesControllerPatch
